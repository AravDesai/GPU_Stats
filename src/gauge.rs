//This uses source code from egui_gauge but with some values changed so it is compatibe with egui 0.27.2

pub mod gauge_at_home {
    ...
    ///
    use eframe::egui::{self, Align2, FontFamily, FontId, Rect, Response, Sense, Shape, Ui};
    ...
    impl Gauge {
        ...
        fn value_to_angle(&self, v: f64) -> i32 {
            ((270.0 - ((v - self.min_value) / (self.max_value - self.min_value)) * 270.0) - 45.0)
                as i32
        }
